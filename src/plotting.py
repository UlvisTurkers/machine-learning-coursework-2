...
def plot_accuracy_vs_budget(
    ...
    # accuracy vs cumulative budget, with optional std shading
    with plt.rc_context(STYLE):
        ...
        for method, data in results_dict.items():
            ...
            y = np.asarray(data["accuracies"]) * 100
...
def plot_comparison_bars(
    ...
    # bar chart comparing methods at a specific budget level
    with plt.rc_context(STYLE):
        methods, accs, errs = [], [], []
        extra_idx = 0
...
def plot_tsne_selection(
    ...
    # t-SNE of all features colored by class, with selected points highlighted
    ...
    with plt.rc_context(STYLE):
        ...
        # all points in the background, lightly colored by class
        for i, lab in enumerate(unique_labels):
            mask = labels == lab
            ax.scatter(emb[mask, 0], emb[mask, 1],
                       c=[cmap(i)], alpha=0.25, s=6, label=f"Class {lab}")

        # selected points shown with bold X markers
...
def plot_training_curves(
    ...
    # loss and accuracy curves during classifier training
